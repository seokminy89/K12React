...
react-router-dom
  ...
  이 경우 주소가 하나로 고정되기 때문에 즐겨찾기와 같은 기능을 
  사용할 수 없다.
  어떤 주소로 들어왔을때 그 주소를 알아내어 그에 해당하는 컴포넌트를
  렌더링 하고, 그 상태를 관리하기 위해 내부적으로 state나 props를
  관리할 수 있게 해주는 도구가 react-router-dom이다.
...
Switch
  ...
  렌더링 한다.
Link
  : a태그를 통한 링크는 클릭할 때마다가 화면이 새로고침되어 갱신된다.
  React는 화면의 깜빡임이 없어야 하므로, Link태그를 사용해 
  링크를 눌렀을 때 reloading되지 않도록 처리해준다.
*/
...
function Topics() {
  //링크를 구성할 li 태그를 저장할 배열 생성
  let liTag = [];
  //배열의 크기만큼 반복
  for(var i =0 ; i <contents.length ; i++){
    //반복횟수만큼 배열의 끝에 li태그를 추가한다.
    liTag.push(
  ...
  return (
    <BrowserRouter>
      <div className="App">
        ...
        {/* 
        Route path="매핑할경로명"
          : 해당 경로로 요청이 들어오면 </Route> 사이에 매핑된 컴포넌트를 렌더링한다.
        exact
          : <Route 컴포넌트에 삽입하면 path(경로)가 정확히 일치하는 경우에만
          라우팅 된다. 만약 exact를 포함하지 않으면 Topics를 눌렀을때 Home이 같이
          보이는 문제가 발생한다.
        */}
        ...
        /가 모든 URL을 포함하는 형태이므로 아래쪽에 정의된 컴포넌트는 
        렌더링 되지 않는다.
        */}
        <Switch>
        <Route exact path="/"><Home></Home></Route>
