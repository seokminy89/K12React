...
class App extends Component {
  ...
  render(){
    ...
    if(this.state.mode==='list'){
      ...
      자식이 부모로 데이터를 전달할때는 Event(onClick이나 onPress등)를 통해 전달해야 한다.
      단 이때 부모가 내려준 props의 기능(함수)를 사용한다.
      */
      ...
      /*
      리스트에서 특정 게시물을 클릭할 때 전달된 파라미터를 통해 mode는
      view로 변경하고 일련번호는 state의 num을 변경한다.
      이때 새롭게 랜더링 되면서 얻어온 num을 viewContents 컴포넌트의 props로 사용한다.
      */
      content = <ViewContents num={this.state.num} myBoardList={(pmode)=>{
        //리스트로 전환하기 위한 props를 정의한다.
        console.log("mode", pmode);
        //파라미터로 전달받은 값을 통해 mode를 변경한 후 랜더링 한다.
      ...
      content = <WriteContents
        //리스트 바로가기 버튼에서 사용할 함수를 props로 전달.
        myBoardList={(pmode)=>{
          console.log("mode", pmode);
          this.setState({mode:pmode});
        }}
