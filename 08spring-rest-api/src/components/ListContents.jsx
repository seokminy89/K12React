...
class ListContents extends Component{
  ...
  state 값이 변경될때마다 render()ㅎ마수가 호출되어 화면이 갱신된다.
  */
  state = {
    blists : [],
    pageNum : 1
  ...
  /*
  render()함수가 호출된 직후 호출되는 수명주기 함수로써
  첫 랜더링에서는 빈 화면을 먼저 출력하고, 서버와 통신 후 
  데이터가 있는 화면을 출력한다.
  */
  componentDidMount(){
    ...
    스프링 서버로 요청한 후 콜백 데이터가 있으면 then()절에서 콜백받아 사용한다.
    */
    fetch('http://localhost:8081/jsonrestapi/restapi/boardList.do?nowPage'+pageNum)
      ...
      .then(function(json){
        ...
        console.log(json);
        //콜백 데이터를 통해 state 값을 변경하면 화면이 갱신된다.
        this.setState({blists:json});

        /*
        this.state.blists = json;
        이와 같이 state를 변경할수도 있으나, 이 경우 render()함수가
        호출되지 않으므로 화면이 갱신되지 않는다.
        반드시 setState()를 통해 state값을 변경해야 render()함수가
        호출되어 화면이 업데이트 된다.
        */
  ...
  render(){
    ...
    for(var i =0 ; i < this.state.blists.length ; i++){
      ...
      /*
      tr태그에 있는 Key는 리액트가 중복되지 않는 Key prop을 요구하므로 삽입된다.
        ...
        <tr align="center" key={row.num}>
          ...
          <td align="left"><a href={row.num} data-id={row.num} onClick={
            (e)=>{
              ...
              e.preventDefault();
              /*
              부모 컴포넌트가 props를 통해 전달한 값(함수)을 통해 
              파라미터를 전달한다. 게시물의 일련번호와 화면의 mode값이 사용된다.
              */
