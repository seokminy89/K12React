...
//클래스형 컴포넌트(함수형 컴포넌트는 HOOK(훅)챕터에서 다룰 예정.)
class App extends Component {
  ...
  constructor(props){
    ...
    //해당 앱에서 사용할 데이터
    /*
    mode를 통해 첫 진입인지 쓰기, 읽기 모드인지 판단한다.
  ...
  render(){
    ...
    if (this.state.mode === 'welcome'){
      ...
      _article = <CreateForm onSubmitValue={function(_title, _desc){
        ...
        //state값을 변경한다. 앞에서 추가한 배열로 교체된다. 그리고 화면이 다시 렌더링 된다.
        this.setState({
          contents : _contents,
          mode : 'read',
    ...
    //Navi 컴포넌트로 배열로 정의된 contents를 props로 전달한다.
    return (
      <div className="App">
        {/* 
          부모가 자식에게 함수기능을 가진 props를 내려준다.
          여기서는 onChangePage라는 props를 정의하였는데.
        ...
        */}
        ...
        //여기서 삭제처리를 하면 렌더링을 한번만 해도 되므로 효율적이다.
          if(btn_mode==='delete'){
            ...
            //mode가 delete가 아닌경우 state변경 후 렌더링 한다.
            this.setState({
              mode : btn_mode
            });
          }
