...
class UpdateForm extends Component{
    ...
    */
    ...
    /* 
    각 input 상자에서 입력한 내용이 있을때 state 값을 변경하기 위해
    선언한 함수로, setState()함수를 통해 값을 변경한다. 이벤트 객체를 통해
    input상자의 name, value속성을 얻어와서 지정된 값을 변경할 수 있다.
    */
          ...
          <article>
              ...
              <form action="/create_process" method="post" onSubmit={(e)=>{
                  ...
                  {/* 
                    ...
                    값을 변경하는것이 불가능하다. props는 읽기 전용 데이터이므로
                    값의 변경은 state를 통해서만 가능하다.
                    이 경우 props로 전달된 데이터를 통해 state를 초기화하고, 각 input상자에서
